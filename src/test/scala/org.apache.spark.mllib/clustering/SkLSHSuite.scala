...
class SkLSHSuite extends FunSuite with MLlibTestSparkContext {
...
/*
  ...
  test("three clusters") {
    ...
    val model = SkLSH.train(data, 3, 9, 1.1, 0.6, 100, 1, K_MEANS_PARALLEL, 29)
    //val rdd_res = model.predict(data).collect()
...
/*
  test("pseudo data test(3 clusters") {
    ...
    val model = SkLSH.train(data, 3, -1, 1.0, 100, 1, K_MEANS_PARALLEL, 29)
    //val rdd_res = model.predict(data).collect()
    // println(model.pointProj.map(i => i._1 + "\n" + i._2).collect.mkString("\n"))
    //System.exit(1)
